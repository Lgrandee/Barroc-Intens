...
<body class="bg-gray-100 text-gray-800">
  ...
  <nav class="bg-white border-y border-gray-200">
    <ul class="p-4 flex gap-4">
      <li>
        <a href="{{ route('customers.index') }}" class="text-blue-600 hover:underline">← Terug naar overzicht</a>
      </li>
      <li>
        <a href="{{ route('customers.edit', $customer->id) }}" class="text-blue-600 hover:underline">✏️ Klant bewerken</a>
      </li>
    </ul>
  </nav>
  ...
  <main class="p-6 max-w-3xl mx-auto">
    ...
    <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
          <div>
            ...
            <p class="text-base text-gray-800 font-medium">{{ $customer->name_company }}</p>
          </div>

          <div>
            <h3 class="text-sm font-medium text-gray-500 mb-1">Contactpersoon</h3>
            <p class="text-base text-gray-800">{{ $customer->contact_person ?? '-' }}</p>
          </div>
        ...
        <div class="space-y-4">
          <div>
            ...
            <p class="text-base text-gray-800">
              <a href="tel:{{ $customer->phone_number }}" class="text-blue-600 hover:underline">{{ $customer->phone_number ?? '-' }}</a>
            </p>
          </div>

          <div>
            <h3 class="text-sm font-medium text-gray-500 mb-1">BKR Nummer</h3>
            <p class="text-base text-gray-800">{{ $customer->bkr_number ?? '-' }}</p>
          </div>
      ...
      <div class="mt-8 pt-6 border-t border-gray-200 flex gap-3">
        <a href="{{ route('customers.edit', $customer->id) }}" class="flex-1 bg-blue-600 text-white py-2.5 rounded-md text-sm font-medium hover:bg-blue-700 transition text-center">
          ✏️ Bewerken
        </a>
        <a href="{{ route('customers.index') }}" class="flex-1 bg-gray-300 text-gray-800 py-2.5 rounded-md text-sm font-medium hover:bg-gray-400 transition text-center">
          Terug
        </a>
      </div>
